...
def download_data():
    ...
    cmumosei_dataset={}
    for key in source:
        cmumosei_dataset[key]=mmdatasdk.mmdataset(source[key],'/esat/smcdata/users/kkontras/Image_Dataset/no_backup/CMU_MOSEI/cmumosei_%s/'%key)
    return cmumosei_dataset

def process_data(folders=["/esat/smcdata/users/kkontras/Image_Dataset/no_backup/CMU_MOSEI/cmumosei_highlevel",
                          "/esat/smcdata/users/kkontras/Image_Dataset/no_backup/CMU_MOSEI/cmumosei_labels"]):
    ...
    for folder in folders:
        cmumosei_dataset[folder.split("/")[-1].split("_")[1]]=mmdatasdk.mmdataset(folder)

    print(cmumosei_dataset.keys())
