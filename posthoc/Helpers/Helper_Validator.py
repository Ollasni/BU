    ...
    def validate(self, data_loader, description, show_border_info=False, skip_modality="full"):
            ...
            with torch.no_grad():
                ...
                for batch_idx, served_dict in pbar:
                    ...
                    target = served_dict["label"].type(torch.LongTensor).to(self.device)
            ...
            #         with open('./conf_vit_uni_test.pkl', 'rb') as f:
            #             self.multi_fold_results = pickle.load(f)
            #     else:
            #         with open('./conf_vit_uni_train.pkl', 'rb') as f:
            #             self.multi_fold_results = pickle.load(f)
            # else:
            #     if description == "Validation":
            ...
            #             self.multi_fold_results = pickle.load(f)
            #     else:
            #         raise ValueError("No CEU file for descriptor {}".format(description))
            ...
            for pred_key in preds[0].keys():
                ...
                metrics["ece_correct"][pred_key] = ece(torch.from_numpy(total_preds_nonargmaxed[pred_key][total_preds[pred_key] == tts]), torch.from_numpy(tts[total_preds[pred_key] == tts]))
                metrics["ece_false"][pred_key] = ece(torch.from_numpy(total_preds_nonargmaxed[pred_key][total_preds[pred_key] != tts]), torch.from_numpy(tts[total_preds[pred_key] != tts]))
