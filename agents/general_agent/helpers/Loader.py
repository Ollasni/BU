...
class Loader():
    ...
    def load_models_n_optimizer(self):
        ...
        # self.agent.best_model = nn.DataParallel(model_class(encs = enc, args = self.agent.config.model.args), device_ids=[torch.device(i) for i in self.agent.config.training_params.gpu_device])
        # self.agent.best_model = model_class(encs = enc, args = self.agent.config.model.args)
        ...
        if self.agent.config.model.get("compile_model", False):
            ...
            # self.agent.best_model.load_state_dict(self.agent.model._orig_mod.state_dict())
        # else:
        #     self.agent.best_model.load_state_dict(self.agent.model.state_dict())

        # if hasattr(torch, "compile"):
        #     self.agent.model = torch.compile(self.agent.model)
    ...
    def sleep_load(self):
        ...
        # self.agent.bias_infuser.reg_logs = self.agent.logs["reg_logs"]
        # self.agent.bias_infuser.ratio_logs = self.agent.logs["ratio_logs"][list(self.agent.logs["ratio_logs"].keys())[0]]
