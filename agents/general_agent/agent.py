...
class Agent():
    ...
    def initialize_logs(self):
        ...
        if self.config.training_params.wandb_disable:
            self.wandb_run = wandb.init(reinit=True, project="balance", config=self.config, mode = "disabled", dir="/tmp/kkontras", name= self.config.model.save_dir.split("/")[-1][:-8])
        else:
            self.wandb_run = wandb.init(reinit=True, project="balance", config=self.config, dir="/tmp/kkontras", name= self.config.model.save_dir.split("/")[-1][:-8] )
